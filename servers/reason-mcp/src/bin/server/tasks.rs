...
use veoveo_reason_mcp::{
    ...
    contract::{
        AnalyzeRecordingRequest, GroundingDetections, RecordingVideoSelection,
...
async fn run_task_inner(
    ...
    let Some(model) = state.catalog.model(&pipeline.model_id).cloned() else {
        fail!(format!("pipeline model `{}` disappeared", pipeline.model_id));
    };
...
pub(super) async fn completed_payload(
        ...
        .map_err(|error| rmcp::ErrorData::internal_error(error.to_string(), None))?
    {
        TaskPayloadState::Completed(payload) => serde_json::from_value(payload).map_err(|error| {
            rmcp::ErrorData::internal_error(format!("invalid persisted reason result: {error}"), None)
        }),
