...
#[derive(Parser)]
#[command(name = "server", about = "Reason MCP server (streamable HTTP)")]
pub(super) struct Args {
    #[arg(long, default_value_t = 8803)]
    pub(super) port: u16,
    ...
    pub(super) reason_runner: PathBuf,
    ...
    pub(super) max_answer_bytes: usize,
...
fn parse_allowed_host(value: &str) -> Result<String, String> {
    ...
    parse_allowed_host_authority(value)
        ...
        .ok_or_else(|| "expected a host authority such as reason-mcp:8803".to_owned())
}
...
fn parse_database_auth_level(value: &str) -> Result<StoreAuthLevel, String> {
    match value.parse::<StoreAuthLevel>() {
        ...
        Ok(_) => Err("reason requires database-scoped SurrealDB credentials".to_owned()),
