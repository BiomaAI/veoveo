...
def normalize_maritime(command: NormalizeCommand) -> tuple[tuple[Path, ...], Path, Path | None]:
    parquet = command.output_dir / "maritime-features.parquet"
    run_tool(
        "ogr2ogr",
    ...
    report = write_quality_report(
        command,
        adapter="maritime",
        checks=[{"name": "exchange_set_normalized", "passed": parquet.is_file()}],
    )
