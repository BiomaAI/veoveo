...
use crate::contract::{
    CaptureDatasetRequest, DurableOperation, ExecuteMissionRequest, RunScenarioRequest,
};
...
impl TaskExtensionHandler for UavSimTaskExtension {
    ...
    async fn start_tool_task(
        ...
        let operation = match request.name.as_str() {
            ...
            "execute_mission" => DurableOperation::ExecuteMission(
                serde_json::from_value::<ExecuteMissionRequest>(arguments)
                    .map_err(|error| AdapterError::invalid_params(error.to_string()))?,
            ),
