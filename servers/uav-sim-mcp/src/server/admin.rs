...
//! authentication as the MCP surface; it establishes no domain
//! administration API.
...
async fn doc_body(Path(doc_id): Path<String>) -> Response {
    match SERVER_DOCS.doc(&doc_id) {
        Some(doc) => (
            ...
            .into_response(),
        None => (StatusCode::NOT_FOUND, "unknown UAV simulation document").into_response(),
    }
}
...
mod tests {
    ...
    async fn llms_txt_indexes_the_embedded_documents() {
        ...
        assert!(body.starts_with("# uav-sim\n"));
