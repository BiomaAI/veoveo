...
#[derive(Parser)]
#[command(name = "server", about = "Optimization MCP server (streamable HTTP)")]
...
fn parse_database_auth_level(value: &str) -> Result<StoreAuthLevel, String> {
    match value.parse::<StoreAuthLevel>() {
        ...
        Ok(_) => Err("optimization requires database-scoped SurrealDB credentials".to_owned()),
...
fn parse_allowed_host(value: &str) -> Result<String, String> {
    ...
    parse_allowed_host_authority(value)
        ...
        .ok_or_else(|| "expected a host authority such as optimization-mcp:8788".to_string())
}
