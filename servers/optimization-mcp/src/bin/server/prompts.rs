    ...
    pub(super) fn render(self, arguments: Option<JsonObject>) -> Result<GetPromptResult, McpError> {
        ...
        let text = match self {
            Self::FormulateRouting => format!(
                "Formulate this routing decision: {objective}. Read {travel_model} and preserve its location order, mobility profile, release pins, unavailable arcs, and cost units. Model service or pickup-delivery orders, but do not mix those order families in one cuOpt 26.08 problem. Give every order, vehicle, vehicle type, location, and capacity dimension a stable id. Make optional service explicit with a positive drop penalty and a positive prize objective weight. Read optimization://profiles, choose {profile}, validate windows and fleet bounds, then invoke optimize_routes as an MCP Task. Treat optimization://solution and its verification subresource as the decision record.",
            ...
            Self::CompareRouteScenarios => format!(
                "Prepare a route-scenario batch answering: {question}. Use these stable cases: {cases}. Keep each case independently complete, materialize the same immutable travel-model inputs before submission, and use one solver profile for the GPU batch. Invoke optimize_route_scenarios as an MCP Task. Compare case objectives, vehicles used, dropped orders, feasibility, termination, and verification findings; do not compare unverified routes as operational equivalents.",
