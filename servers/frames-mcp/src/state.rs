...
impl FramesState {
    ...
    pub async fn list_frames(&self, scope: &FrameScope) -> Result<Vec<RrdFrameDefinition>> {
    ...
    pub async fn insert_frame(&self, scope: &FrameScope, frame: RrdFrameDefinition) -> Result<()> {
...
fn frame_key(frame: &RrdFrameDefinition) -> Result<FrameId> {
    FrameId::new(frame.frame_id.as_str())
        .with_context(|| format!("frame `{}` is not a Frames resource id", frame.frame_id))
}
