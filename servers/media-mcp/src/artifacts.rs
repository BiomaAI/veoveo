//! Artifact access for media-mcp, backed by the shared artifact plane.
//!
//! media no longer owns a private bucket or artifact metadata tables. It calls
//! the central artifact service with the caller's forwarded gateway identity;
//! the plane stamps tenant/owner and records authorization state,
...
impl ArtifactRepository {
    ...
    pub async fn put(
        ...
        let (request, bytes) = put_request(artifact);
        self.plane
    ...
    pub async fn put_with_capability(
        ...
        let (artifact, bytes) = put_request(artifact);
...
fn put_request(artifact: ArtifactPut) -> (PutArtifactRequest, Vec<u8>) {
