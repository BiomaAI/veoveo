...
host compatibility. Presentation validates the receipt index and displays each check's
source status without treating stale host-local evidence as a failed GitHub build.
Local `show` and release `verify` retain their failure checks. The summary's locked
Cargo metadata query may download uncached dependency sources to resolve the complete
input graph; it does not require a prewarmed offline
