...
| App presentation metadata changed in one MCP server while the deployment closure retained an older Console frontend | the Map image correctly emitted `prefersBorder: false`, but the live Console bundle had no full-workspace rendering branch and continued to constrain the App to 420 px; diagnosing the incomplete closure and staging the missing Console image added 139.46 s after the first rollout | derive affected image targets from both ends of a changed cross-component contract, reject a staged closure whose Console bundle predates an App presentation capability, and verify the live catalog plus rendered presentation branch before declaring the rollout complete |
