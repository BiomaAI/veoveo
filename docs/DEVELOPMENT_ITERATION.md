...
| Recording Deployments were scaled to zero while their HelmReleases remained active | Flux drift correction restored Hub and the simulator during the bounded reset, producing another 2,020 receipts plus a final 261 MiB spool tail | suspend the owning HelmReleases before scaling writers down, prove both desired and current replicas remain zero, then resume reconciliation only after the compatible Git revision is published |
...
| Sealing removed capture materialization and Blueprint staging but retained the recording-scoped live static context | Artifact-backed replay passed after complete cache loss, yet an 11,595-byte `.static-context` file for the sealed segment remained in the Hub spool | derive every static-context path from confined committed capture-layer metadata, remove it after the durable seal transition, retry cleanup on idempotent seal, and assert the path cannot escape the spool |
