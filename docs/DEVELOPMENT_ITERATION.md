...
| A failed release delays the Git commit that fixes it | Map's failed image holds a 20-minute Helm health check while the root Kustomization waits up to 30 minutes; the corrected digest is already fetched | enable the pinned Flux controllers' health-check cancellation on a new revision, preserve remediation, and measure an isolated unhealthy-to-fixed transition |
