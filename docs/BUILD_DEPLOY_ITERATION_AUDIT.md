...
| Feature unification hid missing schema features | Gateway and browser-edge tests compiled the new DTOs, while the isolated contract check failed because UUID and timestamp schema features were supplied only by other consumers. Explicit `uuid1` and `chrono04` features fix the owning contract. | Check shared contracts in isolation before broad consumer checks. Keep schema feature ownership in the crate that derives the schema. |
| Workspace assets have a small independent build loop | The first Vite production solve takes 1.64 s. After splitting the conversation renderer and participant panel, the warm solve takes 0.30 s; entry JavaScript is 105 KB gzip and the conversation chunk is 74 KB gzip. Rust is not involved. | Load conversation components on demand and keep their Docker stage independent of Console assets and Rust compilation. |
