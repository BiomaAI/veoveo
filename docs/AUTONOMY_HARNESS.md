...
| Retry behavior | Retry only operations whose qualified protocol contract proves safety. Use the provider completion/recovery profile in [contract evolution](CONTRACT_EVOLUTION.md#ce-01-provider-completion-follows-qualified-semantics). | Configure authenticated completion, idempotency identities, bounded recovery, and incident handling for unresolved effects. | Transport failure cannot duplicate a non-idempotent action; observation recovery preserves operation identity and resource fencing. |
