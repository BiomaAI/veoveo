...
| `platform/computers/src/session_grants/` and `queries/*session_grant*` | durable browser tickets, bounded owner inventory, installation grant limits, one-use redemption, current family/policy/run renewal and parent-bound revocation; service terminal composition lives in `servers/computers-mcp`; CLI and agent grants remain delivery work |
