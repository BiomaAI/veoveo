...
| `platform/computers/src/commands/` and `queries/{queue,dispatch}_command.surql` | encrypted admission, grant/policy fences, distinct execution slots, metadata-only Tasks, one-shot dispatch, original-run containment and interruption settlement; successful output and public execution remain work |
| `platform/computers/src/command_secrets/` | private queued-command encryption, immutable run/actor binding and retained-key request comparison; the same qualified guest frame carries command bytes |
