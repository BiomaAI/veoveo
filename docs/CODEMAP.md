...
| [`platform/store/src/workspace/runs/`](../platform/store/src/workspace/runs/DESIGN.md) | per-chat agent admission, atomic human-turn participation and same-chat typed replies in `workspace/participation.rs`, server-captured quotes qualified by `tests/workspace/replies.rs`, bounded concurrent runs, fixed context, execution fences, cancellation and interrupted-worker recovery |
