...
| [`deploy/helm/veoveo/DESIGN.md`](../deploy/helm/veoveo/DESIGN.md) | chart-owned Computers core/capacity selection, independent control and private host deployments, retained PVC, trust references and host network policy |
...
| `platform/computers/contract/src/files.rs` | pending public regular-file handoff types and generated schemas; bounded import/export intent, canonical retained-relative paths and Artifact result metadata; activation belongs to the domain/service integration |
...
| `platform/computers/src/files/` | private file authority, encrypted request and capability preparation, one-shot dispatch, live continuation, bounded original-run containment, result settlement and recoverable Task delivery; metadata-only public Task authority and supervised service integration; installed qualification remains pending |
| [`platform/computers/execution/`](../platform/computers/execution/DESIGN.md) | private framed argv codec and packaged guest launcher; fixed provider command, confined launch-directory resolution and finite stdin; `files.rs`, `file_io.rs` and `file_confinement.rs` own the local regular-file helper pending public integration; named agent authority and Task integration live in the Computers domain and service |
...
| `platform/computers/src/session_grants/` and `queries/*session_grant*` | durable browser tickets, bounded owner inventory, installation grant limits, one-use redemption, current family/policy/run renewal and parent-bound revocation; service terminal composition lives in `servers/computers-mcp`; agent delegation remains delivery work |
...
| [`apps/console/bff/src/workspace/`](../apps/console/bff/src/workspace/DESIGN.md) | shared browser edge for the separate Workspace client: typed chat routes, cookie-only credentials, bounded responses and CSRF-protected mutations |
...
| [`platform/gateway/src/bin/gateway/workspace/`](../platform/gateway/src/bin/gateway/workspace/DESIGN.md) | direct-human, current Work Context admission and bounded Workspace application routes; browser edge and stream qualification remain delivery work |
...
| [`apps/workspace/`](../apps/workspace/DESIGN.md) | independent daily-productivity client and generated chat contracts; renderer, capabilities and installed acceptance remain active implementation work |
...
| `servers/computers-mcp/src/server/cli/` | retained narrow-credential admission, exact connection fencing, five-method stock CLI gRPC facade and bounded WebSocket byte pump; public SSO/ingress qualification remains delivery work |
