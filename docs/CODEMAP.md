...
| `platform/computers/src/cli_grants/` and `queries/*cli*` | private stock-CLI pairing ledger, named session-bound grants, shared browser/CLI quota, separately fenced connection leases and owner revocation; public pairing/tunnel integration remains delivery work |
...
| `servers/computers-mcp/src/server/terminal/`, `access_events.rs` and `runtime_access.rs` | exact-Origin one-use browser attachment, shared bounded revocation wakes, current domain-to-runtime lease renewal and provider resource/process verification before terminal bytes |
