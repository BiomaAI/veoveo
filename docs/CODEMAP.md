...
| `platform/computers/src/commands/` and `queries/{queue,dispatch}_command.surql` | encrypted admission, grant/policy fences, distinct execution slots, metadata-only Tasks, one-shot dispatch, original-run containment, interruption settlement and durable output-capability preparation; successful output and public execution remain work |
