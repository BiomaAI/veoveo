...
export function attachWorkspaceApp(frame: HTMLIFrameElement, app: AppDescriptor, chat: string, changed: () => void, navigate: (uri: string) => boolean, failed: (message: string) => void) {
  if (!frame.contentWindow) throw new Error("The App frame is not ready.");
  ...
  inner.onmessage = (message, extra) => {
    ...
    const act = async (): Promise<Result> => {
      ...
      if (typeof id !== "string" || !id || id.length > 4096) throw new Error("A valid Task identity is required.");
      ...
      if (!responses || typeof responses !== "object" || Array.isArray(responses)) throw new Error("Input responses must be an object.");
    ...
    void act().then(result => inner.send({ jsonrpc: "2.0", id: message.id, result }), error => inner.send({ jsonrpc: "2.0", id: message.id,
      error: { code: -32603, message: error instanceof Error ? error.message : "The Task request could not be confirmed." } })).catch(() => {});
  };
  ...
  bridge.oncalltool = async ({ name, arguments: args, inputResponses, requestState }) => {
    if (!app.tools.some(tool => tool.name === name)) throw new Error("This tool is not available to this App.");
  ...
  bridge.onopenlink = async ({ url }) => {
    if (url.startsWith("ui://")) {
      if (!navigate(url)) throw new Error("This App is not available with your access.");
      return {};
    }
  ...
  void bridge.connect(subscriptions.transport).catch(error => { if (!abort.signal.aborted) failed(error instanceof Error ? error.message : "The App connection failed."); });
