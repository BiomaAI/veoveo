...
import type { InputAnswer, OperationSummary, OperationView } from "./generated/workspace.ts";
...
export function Activity({ chat, close }: { chat?: string; close?: () => void }) {
  ...
  return <section className={chat ? "task-panel" : "activity-page"} aria-label={chat ? "Your activity in this chat" : "My activity"}>
    ...
    <div className="task-list">{items.slice(0, visible).map(operation => <TaskCard key={operation.id} operation={operation} observed={observed}/>)}</div>
...
function TaskCard({ operation, observed }: { operation: OperationSummary; observed: (id: string, active: boolean) => void }) {
