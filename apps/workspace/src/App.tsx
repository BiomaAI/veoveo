...
import { ArrowRight, Bell, LogOut, Menu, MessageSquare, Plus, Users, X } from "lucide-react";
...
function Workspace({ session }: { session: WorkspaceBootstrap }) {
  ...
  function select(id?: string) {
    setSelected(id); setInbox(false); setMobileNav(false);
    history.pushState(null, "", id ? `/workspace/?chat=${id}` : "/workspace/");
  }
  useEffect(() => { const pop = () => setSelected(selectedChat()); window.addEventListener("popstate", pop); return () => window.removeEventListener("popstate", pop); }, []);
  ...
  return <div className={`workspace ${mobileNav ? "show-nav" : ""}`}>
    <aside className="sidebar"><button className="wordmark" onClick={() => select()}>veoveo<span>Workspace</span></button>
      ...
      <button className={`nav-item ${inbox ? "active" : ""}`} onClick={() => { setInbox(true); setMobileNav(false); void invitations.refetch(); }}><Bell size={16}/> Invitations {!!invitations.data?.length && <span className="count">{invitations.data.length}</span>}</button>
      <div className="sidebar-section">YOUR CHATS <span>{chats.data?.length ?? 0}</span></div>
      <nav className="chat-list" aria-label="Chats">{chats.data?.map(chat => <button key={chat.id} className={`chat-item ${selected === chat.id && !inbox ? "active" : ""}`} onClick={() => select(chat.id)}>
    ...
    <main className="main"><div className="mobile-top"><button className="icon-button" aria-label="Open chats" onClick={() => setMobileNav(!mobileNav)}><Menu size={20}/></button><span>Veoveo Workspace</span></div>
      {inbox ? <div className="inbox"><span className="eyebrow">YOUR WORKSPACE</span><h1>Invitations</h1><p className="muted">Choose the conversations you want to join.</p>
...
function Room({ chat, session, changed }: { chat: string; session: WorkspaceBootstrap; changed: () => void }) {
  ...
  return <><header className="chat-header"><div><h1>{room.snapshot.chat.title}</h1><p><span className={`status-dot ${room.connected ? "online" : ""}`}/>{room.connected ? "Connected" : "Reconnecting"}<span>·</span>{room.snapshot.members.filter(member => member.active).length} people{room.snapshot.chat.archived && " · Archived"}</p></div><button className={details ? "active" : ""} onClick={() => setDetails(!details)}><Users size={16}/> Participants</button></header>
