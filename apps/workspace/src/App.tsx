...
export default function App() {
  ...
  if (!session.data || (session.error instanceof ApiError && [401, 403, 404].includes(session.error.status))) {
    const status = session.error instanceof ApiError ? session.error.status : undefined;
    const [heading, detail] = status === 401 ? ["Sign in to your workspace", "Your session ended, or you haven't signed in yet."]
      : status === 403 ? ["You don't have access to this workspace", "Your account isn't a member of this Work Context. Ask an administrator to add you."]
      : status === 404 ? ["This workspace wasn't found", "Check the address, or ask an administrator which workspace to use."]
      : ["Your workspace couldn't open", session.error?.message ?? "Try again in a moment."];
    return <main className="entry"><div className="wordmark">veoveo<span>Workspace</span></div><h1>{heading}</h1><p role={status === 401 ? undefined : "alert"}>{detail}</p>
      {status === 401 ? <a className="primary" href={loginPath()}>Sign in <ArrowRight size={17}/></a> : <button onClick={() => void session.refetch()}>Try again</button>}
    </main>;
  }
...
function Workspace({ session }: { session: WorkspaceBootstrap }) {
  ...
  async function decide(invitation: Invitation, state: "accepted" | "declined") {
    ...
    catch (error) { setError(error instanceof Error ? error.message : "Veoveo couldn't update the invitation. Try again."); }
    finally { setBusy(false); }
  }
  return <PersonalUpdates.Provider value={personal}><div className={`workspace ${mobileNav ? "show-nav" : ""}`}>
    <aside className="sidebar"><button className="wordmark" onClick={() => select()}>veoveo<span>Workspace</span></button>
      ...
      <button className="new-chat" disabled={!session.canContribute} title={session.canContribute ? undefined : "You have view-only access. Ask an administrator for contributor access to start chats."} onClick={() => { setNewChat(true); setError(undefined); }}><Plus size={17}/> New chat</button>
      ...
      <button className={`nav-item ${activity ? "active" : ""}`} title={personal.attention ? `${personal.attention} ${personal.attention === 1 ? "task needs" : "tasks need"} your input` : "Your private activity"} onClick={() => { setActivity(true); setAgents(false); setComputers(false); setInbox(false); setMobileNav(false); history.pushState(null, "", "/workspace/?view=activity"); }}><ActivityIcon size={16}/> My activity {!!(personal.attention + personal.unread) && <span className="count">{personal.attention + personal.unread}</span>}</button>
      {(personal.attention > 0 || personal.unread > 0) && <small className="activity-notice" role="status">{personal.attention ? `${personal.attention} ${personal.attention === 1 ? "task needs" : "tasks need"} your input` : `${personal.unread} new activity ${personal.unread === 1 ? "update" : "updates"}`}</small>}
