...
export function InstanceManager({ api, authoring, definitions, refreshVersion }: { api: AgentApi; authoring: Authoring; definitions: readonly Definition[]; refreshVersion: number }) {
  ...
  return <section className="am-instances" aria-label="Managed instances"><header className="am-heading"><div><h2>Managed instances</h2><p>Desired state records your request. Observed state reports provisioning and runtime convergence.</p></div><button onClick={() => void refresh()}>Refresh instances</button></header>
