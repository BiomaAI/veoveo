...
#[cfg(test)]
use run_evidence::parse_publication_index_digests;
...
pub(crate) fn prepare_with_builder(
    ...
    let _timing = operation::span(operation::Phase::Planning);
...
pub(crate) fn execute(
    ...
    let solve_timing = operation::span(operation::Phase::Solve);
    let result = buildkit::execute(&mut command, evidence.buildkit_trace_path());
    drop(solve_timing);
    operation::cpu_delta(
