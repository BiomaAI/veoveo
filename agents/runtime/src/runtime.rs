...
//!
//! Lease, wake, Task, input-request and recovery operations share the same fence.
//! Episode transactions and managed readiness live in focused child modules.
...
pub struct AgentRuntime {
    ...
    managed: Option<crate::ManagedRuntimeBinding>,
}
...
impl AgentRuntime {
    ...
    pub async fn agent_record(&self) -> Result<AgentRecord> {
        select_only(&self.store, self.agent_id.record_id(), "agent").await
    }

    pub async fn episode_record(&self, episode_id: AgentEpisodeId) -> Result<AgentEpisodeRecord> {
