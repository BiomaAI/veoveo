...
# Start a Cloudflare quick tunnel. Copy the printed URL into `just compose-up <url>` or `just e2e <url>`.
tunnel protocol='http2':
    cloudflared tunnel --protocol {{protocol}} --url http://localhost:8787
