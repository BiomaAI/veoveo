...
use crate::components::canonical_runtime_components;
use crate::{
    GpuRuntimeRequirement, RuntimeComponent, RuntimeComponentVersion, SimulationContractError,
};
...
impl SimulationRuntimeBuildLock {
    ...
    pub fn validate(&self) -> Result<(), SimulationContractError> {
        ...
        if components != required_components {
            return Err(SimulationContractError::RuntimeComponents {
        ...
        for source in &self.sources {
            if !source_components.insert(source.component) {
                return Err(SimulationContractError::Duplicate {
...
impl SimulationRuntimeReleaseEvidence {
    ...
    pub fn validate(&self) -> Result<(), SimulationContractError> {
        if self.base_image.kind != ArtifactKind::OciImage {
            return Err(SimulationContractError::ArtifactKind {
...
) -> Result<BTreeSet<RuntimeComponent>, SimulationContractError> {
