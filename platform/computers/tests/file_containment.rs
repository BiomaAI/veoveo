...
async fn dispatched(
    ...
    drop(ticket); // Native file outcome is unknown to this restarted fixture worker.
    (a, b, owner, claim, grant.grant_id)
}
fn stopped(operation: &FileOperation) -> ReachedState {
...
async fn cancelled_file_is_settled_only_after_original_run_termination() {
    ...
    assert!(
        a.abort_queued_file(&claim, FileRefusal::AuthorityDenied)
            .await
            .is_err()
    );
    assert!(
        a.begin_file_containment(&claim, FileInterruption::Cancelled)
            .await
            .is_err()
    );
    assert!(
        a.begin_file_containment(&claim, FileInterruption::Deadline)
            .await
            .is_err()
    );
    ...
    .unwrap();
    let containing = a
        .begin_file_containment(&claim, FileInterruption::Cancelled)
        .await
        .unwrap();
    assert_eq!(containing.stage(), FileTransferStage::Containing);
    assert!(containing.outcome().is_none());
    let (left, right) = futures::join!(a.admit_file_stop(&claim), b.admit_file_stop(&claim));
    assert_eq!(
    ...
    assert!(b.admit_file_stop(&successor).await.unwrap().is_none());
    let ticket = read_ticket(&b, &successor).await;
    let observed = stopped(ticket.operation());
    let terminal = b
        .complete_file_containment_read(&successor, ticket, observed)
        .await
        .unwrap();
    assert_eq!(terminal.stage(), FileTransferStage::Cancelled);
    assert_eq!(
        terminal.outcome(),
        Some(FileOutcome::Terminated(FileInterruption::Cancelled))
    );
...
async fn an_owner_stop_can_abort_before_containment_gets_its_first_stop_ticket() {
    ...
    a.abort_undispatched(&stop_claim, UndispatchedOutcome::CancelledBeforeDispatch)
        .await
        .unwrap();
    let ticket = a.admit_file_stop(&claim).await.unwrap().unwrap();
    let reached = stopped(ticket.operation());
    let terminal = a.complete_file_stop(&claim, ticket, reached).await.unwrap();
    assert_eq!(
        terminal.outcome(),
        Some(FileOutcome::Terminated(FileInterruption::ExecutionUnknown))
    );
    assert_eq!(terminal.stage(), FileTransferStage::Failed);
    assert_eq!(slots(&db).await, 0);
}
...
async fn containment_does_not_clear_an_independent_owner_stop_fence() {
    ...
    a.complete_file_containment_read(
        &claim,
        read,
        stopped(&a.file_for_claim(&claim).await.unwrap()),
    )
    .await
    .unwrap();
...
async fn wrong_run_evidence_and_exhausted_reads_keep_the_slot_across_workers() {
    ...
    a.begin_file_containment(&claim, FileInterruption::ExecutionUnknown)
        .await
        .unwrap();
    let stop = a.admit_file_stop(&claim).await.unwrap().unwrap();
    let mut wrong = stopped(stop.operation());
    wrong.process_id = "replacement-run".into();
    assert!(matches!(
        a.complete_file_stop(&claim, stop, wrong).await,
    ...
    for _ in 0..8 {
        ...
        assert!(
            a.complete_file_containment_read(&claim, ticket, wrong)
                .await
                .is_err()
        );
    }
    ...
    let claim = TaskRuntime::new(db.b.clone(), "computers", "successor")
        ...
        .unwrap();
    assert!(matches!(
        b.admit_file_containment_read(&claim).await.unwrap(),
    ...
    assert_eq!(slots(&db).await, 1);
    assert_eq!(
        b.file_for_claim(&claim).await.unwrap().stage(),
    ...
    assert!(b.admit_file_stop(&claim).await.is_err());
    let mut events = db
        .a
        .client()
        .query(
            "SELECT * FROM outbox_event WHERE event_type = 'computer.file_transfer_recovery_required';",
        )
        .await
        .unwrap()
        .check()
...
async fn queued_cancellation_releases_only_an_undispatched_slot() {
    ...
    TaskRuntime::new(db.a.clone(), "computers", "client")
        ...
        .unwrap();
    let terminal = a
        .abort_queued_file(&claim, FileRefusal::CancelledBeforeDispatch)
        .await
        .unwrap();
    assert_eq!(
        terminal.outcome(),
        Some(FileOutcome::Undispatched(
    ...
    assert!(a.acknowledge_file_task(&terminal).await.is_err());
    let tasks = TaskRuntime::new(db.a.clone(), "computers", "file-worker");
    tasks
        .transition(
        ...
        .await
        .unwrap();
    a.acknowledge_file_task(&terminal).await.unwrap();
    assert_eq!(a.pending_file_transfers(None, 100).await.unwrap().len(), 1);
    ...
    assert!(a.ensure_file_task(&terminal).await.is_err());
}
...
async fn containment_cannot_stop_or_settle_a_replacement_run() {
    ...
    db.a.client()
        ...
        .check()
        .unwrap();
    assert!(a.admit_file_stop(&claim).await.is_err());
    let read = read_ticket(&a, &claim).await;
    assert!(
        a.complete_file_containment_read(&claim, read, stopped(&file))
            .await
            .is_err()
    );
    let current = a.get(owner.owner(), file.computer_id()).await.unwrap();
