...
// Existing qualified 3.2.4 platform image. No mounts or installation credentials.
const IMAGE: &str =
...
impl TestDb {
    pub async fn new() -> Self {
        let name = format!("veoveo-native-computers-test-{}", Uuid::now_v7().simple());
        let mut bytes = [0; 32];
        ...
        let database = format!("computers_{}", Uuid::now_v7().simple());
        ...
        getrandom::fill(&mut bytes).unwrap();
        let runtime_password = hex::encode(bytes);
        admin
            .replace_database_editor("computer_test_runtime", &runtime_password.clone().into())
            .await
            .unwrap();
        let config = StoreConfig::builder(
            &endpoint,
            "computers_tests",
            database,
            StoreCredentials::database("computer_test_runtime", runtime_password),
        )
        .build()
        .unwrap();
