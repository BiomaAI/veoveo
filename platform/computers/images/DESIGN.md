...
| CA bootstrap | Snapshot package `ca-certificates` 20260223, SHA-256 `f7025ab9b24cd73215510931037b02d6960d89584d0d00afba81851abdbe6ef1`; its Mozilla roots enable verified HTTPS before APT installs the version selected by the fixed archive |
...
`ubuntu.sources` replaces every base-image APT source with the exact HTTPS snapshot
URI. Archive signatures remain mandatory. Historical release metadata may outlive
its validity timestamp, so only the freshness deadline is disabled. APT's additional
snapshot selection is disabled because the URI already identifies the archive; no
live archive or security mirror participates in package resolution. The `packages`
build stage allows native package qualification without recompiling the guest helper.
