...
use veoveo_rrd::ingest_parts::{
    VIDEO_STREAM_MARKER, ingest_part_paths, ingest_segment_parts_directory,
};
...
fn authenticated_ingest_stream_id(path: &Path) -> Option<RecordingIngestStreamId> {
    path.ancestors().find_map(|ancestor| {
        ...
        let (_, suffix) = name.split_once(".ingest-")?;
...
fn payload_format_name(value: i32) -> Result<&'static str> {
    match RerunPayloadFormat::try_from(value) {
        Ok(RerunPayloadFormat::Rrd0381) => Ok("rrd_0_38_1"),
...
fn prepare_segment_freeze(
    ...
    veoveo_rrd::segment::SegmentInspection,
)> {
