...
#[test]
fn fork_workload_owns_its_live_view() {
    let repository = Path::new(env!("CARGO_MANIFEST_DIR")).join("../..");
    let path = repository.join("testing/fixtures/fork-installation/deployment.json");
    let loaded = LoadedProfile::load(&path, &repository).expect("load fork workload profile");
    let required = loaded
    ...
    images.push(PlannedImage {
        ...
        reference: "registry.example.internal/veoveo/anonymous-simulation-mcp:fixture".to_owned(),
    });
    loaded
        .validate_image_plan(&images)
        .expect("validate source-qualified platform and workload plan");
