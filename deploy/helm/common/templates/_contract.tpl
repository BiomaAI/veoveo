{{/*
Internal shared Veoveo chart helpers.
All helpers take an explicit dictionary so the library does not prescribe a consumer
chart's values shape.
*/}}

{{- define "veoveo-common.requireInstallation" -}}
{{- $installation := required "Veoveo shared helpers require installation" .installation -}}
...
{{- define "veoveo-common.requireComponent" -}}
{{- $component := required "Veoveo shared helpers require component" .component -}}
...
{{- define "veoveo-common.installationSelector" -}}
veoveo.ai/installation: {{ include "veoveo-common.requireInstallation" . | quote }}
{{- end -}}

{{- define "veoveo-common.componentSelector" -}}
{{ include "veoveo-common.installationSelector" . }}
app.kubernetes.io/component: {{ include "veoveo-common.requireComponent" . | quote }}
{{- end -}}

{{- define "veoveo-common.selectorLabels" -}}
app.kubernetes.io/name: {{ required "Veoveo shared helpers require name" .name | quote }}
app.kubernetes.io/instance: {{ required "Veoveo shared helpers require releaseName" .releaseName | quote }}
{{ include "veoveo-common.componentSelector" . }}
{{- end -}}

{{- define "veoveo-common.labels" -}}
{{ include "veoveo-common.selectorLabels" . }}
app.kubernetes.io/managed-by: {{ required "Veoveo shared helpers require managedBy" .managedBy | quote }}
helm.sh/chart: {{ required "Veoveo shared helpers require chart" .chart | quote }}
app.kubernetes.io/part-of: veoveo
{{- end -}}

{{- define "veoveo-common.image" -}}
{{- $image := required "Veoveo shared image helper requires image" .image -}}
...
{{- define "veoveo-common.podSecurityContext" -}}
fsGroup: 10001
...
{{- define "veoveo-common.containerSecurityContext" -}}
...
{{- define "veoveo-common.gpuRequest" -}}
...
{{- define "veoveo-common.gpuReplicas" -}}
...
{{- define "veoveo-common.gpuPodClaim" -}}
...
{{- define "veoveo-common.gpuResources" -}}
...
claims:
  - name: veoveo-gpu
    request: {{ include "veoveo-common.gpuRequest" . | quote }}
{{- else -}}
{{- toYaml $resources -}}
{{- end -}}
{{- end -}}

{{- define "veoveo-common.platformEnv" -}}
...
{{- define "veoveo-common.httpProbes" -}}
startupProbe:
  httpGet:
...
{{- define "veoveo-common.bootstrapVolumeMount" -}}
- name: bootstrap
  mountPath: /etc/veoveo/bootstrap
  readOnly: true
{{- end -}}

{{- define "veoveo-common.bootstrapVolume" -}}
- name: bootstrap
  configMap:
