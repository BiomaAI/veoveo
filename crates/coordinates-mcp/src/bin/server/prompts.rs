...
#[derive(Debug, serde::Deserialize, schemars::JsonSchema)]
struct GeofenceReviewArgs {
    geofence_uri: Option<String>,
    mission: String,
}
...
pub(super) enum CoordinatesPrompt {
    FrameAudit,
    LocalFrameSelect,
    GeofenceReview,
    TransformExplain,
...
impl CoordinatesPrompt {
    ...
    pub(super) fn name(self) -> &'static str {
        match self {
            Self::FrameAudit => "coordinates-frame-audit",
            Self::LocalFrameSelect => "coordinates-local-frame-select",
            Self::GeofenceReview => "coordinates-geofence-review",
            Self::TransformExplain => "coordinates-transform-explain",
        }
    }
    ...
    fn title(self) -> &'static str {
        match self {
            ...
            Self::GeofenceReview => "Geofence review",
    ...
    fn description(self) -> &'static str {
        match self {
            ...
            Self::GeofenceReview => "Review geofence and mission geometry before execution.",
            Self::TransformExplain => {
    ...
    fn arguments(self) -> Vec<PromptArgument> {
        match self {
            ...
            Self::GeofenceReview => vec![
                optional_arg(
                    "geofence_uri",
                    "Optional coordinates:// geofence or operation URI.",
                ),
                required_arg("mission", "Mission geometry or path to review."),
            ],
            Self::TransformExplain => vec![required_arg(
                "operation_id",
                "Coordinate operation id from coordinates://operation/{operation_id}.",
            )],
        }
    }
    ...
    pub(super) fn render(self, arguments: Option<JsonObject>) -> Result<GetPromptResult, McpError> {
        match self {
                ...
                Ok(prompt_text(
                    ...
                    format!(
                        "Audit this mission for coordinate safety:\n\n{}\n\n\
                         Identify every frame, CRS, datum, unit, axis order, local-frame origin, \
                         body-frame convention, and approximation assumption. Recommend exact \
                         coordinates-mcp tools/resources to call before execution.",
                        args.mission
                    ),
                ))
            }
            ...
            Self::GeofenceReview => {
                let args: GeofenceReviewArgs = parse_prompt_args(self.name(), arguments)?;
                Ok(prompt_text(
                    self.description(),
                    format!(
                        "Review this geofence/mission geometry before execution.\n\n\
                         Geofence URI: {}\n\
                         Mission: {}\n\n\
                         Check frame consistency, polygon closure, path/geofence rule, and \
                         whether validate_geofence should run after any CRS or frame transform.",
                        args.geofence_uri.as_deref().unwrap_or("not specified"),
                        args.mission
                    ),
                ))
            }
            Self::TransformExplain => {
                ...
                Ok(prompt_text(
                    self.description(),
                    format!(
                        "Read coordinates://operation/{} and explain the transform chain, \
                         source/target frame, CRS/datum assumptions, engine, grid packages, \
