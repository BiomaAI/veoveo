...
use veoveo_perception_mcp::contract::{AnalyzeRecordingRequest, ExtractClipRequest};
...
use super::tasks::{PerceptionTaskInput, start_perception_task};

#[derive(Clone)]
pub(super) struct PerceptionTaskExtension {
    state: Arc<AppState>,
}

impl PerceptionTaskExtension {
...
impl TaskExtensionHandler for PerceptionTaskExtension {
    ...
    async fn start_tool_task(
        ...
        let input = match request.name.as_str() {
            "analyze_recording" => PerceptionTaskInput::Analyze(
                serde_json::from_value::<AnalyzeRecordingRequest>(arguments)
                    .map_err(|error| AdapterError::invalid_params(error.to_string()))?,
            ),
            "extract_clip" => PerceptionTaskInput::ExtractClip(
                serde_json::from_value::<ExtractClipRequest>(arguments)
                    .map_err(|error| AdapterError::invalid_params(error.to_string()))?,
            ),
            _ => return Ok(None),
        };
